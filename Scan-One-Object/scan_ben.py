...
class Scan():
    ...
    def scan(self):
        ...
        for iebin, ebin in tqdm(enumerate(np.arange(self.emin,self.emax+1)), disable = 1 - self.verbose):
            ...
            # Add the data in
            data_mu = np.mean(data[ROI])
            data_err_mu = np.sqrt(np.mean(data[ROI]))

            output_norms[iebin,0,0] = data_mu/exp_mu*self.emid[iebin]**2/self.de[iebin]
            output_norms[iebin,0,1] = data_err_mu/exp_mu*self.emid[iebin]**2/self.de[iebin]
            ...
            output_norms[iebin,1,0] = m.values['p8']*dif_mu/exp_mu*self.emid[iebin]**2/self.de[iebin]
            output_norms[iebin,1,1] = m.errors['p8']*dif_mu/exp_mu*self.emid[iebin]**2/self.de[iebin]

            output_norms[iebin,2,0] = m.values['iso']*iso_mu/exp_mu*self.emid[iebin]**2/self.de[iebin]
            output_norms[iebin,2,1] = m.errors['iso']*iso_mu/exp_mu*self.emid[iebin]**2/self.de[iebin]

            output_norms[iebin,3,0] = m.values['psc']*psc_mu/exp_mu*self.emid[iebin]**2/self.de[iebin]
            output_norms[iebin,3,1] = m.errors['psc']*psc_mu/exp_mu*self.emid[iebin]**2/self.de[iebin]

        np.save(self.save_dir + 'spec_ben_o'+str(self.iobj)+self.mc_tag, output_norms)
