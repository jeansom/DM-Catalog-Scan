...
'''
            ...
            batch3 = '''
echo "#!/bin/bash \necho i = \$1 \npython scan_interface.py --catalog_file $catalog_file --start_idx $start_idx --perform_scan 0 --imc ''' + str(mci)+ ''' --iobj \$1 --load_dir DarkSky_sid_loc''' + str(iloc) + ''' --save_dir DarkSky_sid_noJerr_loc''' + str(iloc) + ''' --float_ps_together 0 --noJprof 1 --Asimov 0 --floatDM 1" > run_scripts/conf/run-DS'''+str(it)+'''-v'''+str(mci)+str(loc)+'''.sh
