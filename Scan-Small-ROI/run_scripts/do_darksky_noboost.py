...
'''
            ...
            batch3 = '''
echo "#!/bin/bash \necho i = \$1 \npython scan_interface.py --catalog_file $catalog_file --start_idx $start_idx --perform_scan 0 --imc ''' + str(mci)+ ''' --iobj \$1 --save_dir DarkSky_sid_loc''' + str(iloc) + ''' --float_ps_together 0 --Asimov 0 --floatDM 1 --boost 0" > run_scripts/conf/run-DS'''+str(it)+'''-v'''+str(mci)+str(loc)+'''.sh
