...
'''
        batch2 ='start_idx='+str(halo_start)+'\n'+'catalog_file=2MRSLocalTully_ALL_DATAPAPER_Planck15_0p24cvirerr_v7.csv'+'\n'
        batch3 = '''
echo "#!/bin/bash \necho i = \$1 \npython scan_interface.py --catalog_file $catalog_file --diff p8 --start_idx $start_idx --perform_scan 0 --perform_postprocessing 1 --imc ''' + str(mci) + ''' --iobj \$1 --save_dir Tully_sigmaconc0p24 --float_ps_together 0 --Asimov 0 --floatDM 1" > ./run_scripts/conf/run-Data-diemer-floatDM-ecut4-'''+str(it)+'''-v'''+str(mci)+'''.sh
chmod u+x ./run_scripts/conf/run-Data-diemer-floatDM-ecut4-'''+str(it)+'''-v'''+str(mci)+'''.sh
'''
        runpart='echo   0-49  ./run_scripts/conf/run-Data-diemer-floatDM-ecut4-'+str(it)+'-v'+str(mci)+'.sh %t  > ./run_scripts/conf/run-Data-diemer-floatDM-ecut4-'+str(it)+'-v'+str(mci)+'.conf'+'\n'+'\n'+'srun --multi-prog --no-kill --wait=0 ./run_scripts/conf/run-Data-diemer-floatDM-ecut4-'+str(it)+'-v'+str(mci)+'.conf'+'\n'+'\n'
