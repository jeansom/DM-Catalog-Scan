...
'''
        ...
        batch3 = '''
echo "#!/bin/bash \necho i = \$1 \npython scan_interface.py --catalog_file $catalog_file --diff p8 --start_idx $start_idx --perform_scan 1 --imc ''' + str(mci) + ''' --use_boost 1 --iobj \$1 --save_dir Tully_useboost --float_ps_together 0 --Asimov 0 --floatDM 1" > ./run_scripts/conf/run-Data-indiv-useboost-'''+str(it)+'''-v'''+str(mci)+'''.sh
chmod u+x ./run_scripts/conf/run-Data-indiv-useboost-'''+str(it)+'''-v'''+str(mci)+'''.sh
'''
        runpart='echo   0-49  ./run_scripts/conf/run-Data-indiv-useboost-'+str(it)+'-v'+str(mci)+'.sh %t  > ./run_scripts/conf/run-Data-indiv-useboost-'+str(it)+'-v'+str(mci)+'.conf'+'\n'+'\n'+'srun --multi-prog --no-kill --wait=0 ./run_scripts/conf/run-Data-indiv-useboost-'+str(it)+'-v'+str(mci)+'.conf'+'\n'+'\n'

        batchn = batch1+batch2 + batch3 + runpart
        fname = "./batch/run-Data-indiv-useboost-"+str(it)+"-v"+str(mci)+".batch"
        f=open(fname, "w")
        f.write(batchn)
        f.close()
