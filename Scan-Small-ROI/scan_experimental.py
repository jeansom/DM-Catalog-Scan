...
class Scan():
    ...
    def scan(self):
        ...
        for iebin, ebin in tqdm(enumerate(np.arange(self.emin,self.emax+1)), disable = 1 - self.verbose):
            ...
            DM_intensity_base = np.sum(DM_template/fermi_exposure)
            print "DM_intensity_base is", DM_intensity_base
            print "mean exposure is", np.mean(fermi_exposure)
            print "template mean is", np.mean(DM_template)
            
            expmean.append(np.mean(fermi_exposure))
            dmint.append(DM_intensity_base)
        ...
        np.save("dmint.npy", dmint)
        np.save("expmean.npy", expmean)
