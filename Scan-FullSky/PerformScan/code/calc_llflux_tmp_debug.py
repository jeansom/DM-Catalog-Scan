...
class calc_llflux:
    def __init__(self,J_map_arr,tag,band_mask,external_data=False,calc_flux_array=False,flux_array_ebin=0,bin_min=-5,bin_max=1,nbins=30):
        ...
        # Make Base dirs
        self.make_dir_extra('/tmp/'+self.tag+'/')
        self.make_dir_extra('/tmp/'+self.tag+'/chains/')
        self.make_dir_extra('/tmp/'+self.tag+'/plots/')
        ...
        print "Pre Norm:"
        for key in self.f1.template_dict.keys():
            print "key:",key
            print "shape:",np.shape(self.f1.template_dict[key])
            print "mean:",np.mean(self.f1.template_dict[key])
        ...
        test = np.zeros(shape=(40,hp.nside2npix(self.nside)))
        for key in ['p7','bubs','iso','ps_model']:
            test += np.array(self.f1.template_dict[key])
    ...
    def make_dir_extra(self, dir_str):
    ...
    def setup_b_instance(self,norm,add_ps_mask=True):
        ...
        if self.use_external_data:
            ...
            print "External Data mean:",np.mean(self.external_data[self.flux_array_ebin])
        else:
    ...
    def calc_flux_array(self):
        ...
        b = self.setup_b_instance(0,add_ps_mask=False)
        ...
        for i in range(len(A_array)):
            ...
            if i == 0:
                ...
                b1 = self.setup_b_instance(A_array[i],add_ps_mask=False)
            else:
