...
def calc_limits(J_map,tag,band_mask,mass):
    ...
    # We have flat priors so posterior is just normalized likelihood
    lvals_int_pn = interp1d(range(301),lvals)
    lnorm = integrate.quad(lambda x: lvals_int_pn(x), 0., 300.)[0]
    lvals /= lnorm

    # Now redo this integral and determine when we get 0.95
    lvals_int = interp1d(range(301),lvals)
    for i in range(1,len(xsecs)):
        intval = integrate.quad(lambda x: lvals_int(x), 0., float(i))[0]
        if intval > 0.95:
            # Figure out what fraction of distance from i-1 to i hits 0.95
            intval2 = integrate.quad(lambda x: lvals_int(x), 0., float(i-1))[0]
